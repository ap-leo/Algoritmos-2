...
{      
   struct pilha *p = (struct pilha*)malloc(sizeof(struct pilha));   /* aloca memoria para a nova pilha */
   if (p == NULL)
      return NULL;
   p->topo = NULL;
...
int push (struct pilha *pilha, int dado)
{
   ...
   if(pilha_tamanho(pilha) < 1048576){
      struct nodo *novoNodo = (struct nodo*)malloc(sizeof(struct nodo));   /* verifica se existe memoria para um novo nodo */
      if (novoNodo == NULL)
         return 0;
